...
class RoutesTestCase(TestCase):
    ...
    def test_model_city_duplicate(self):
        try:
            ...
            self.assertEqual({'name': ['Такой город уже существует.']}, e.message_dict)
    ...
    def test_model_train_duplicate(self):
        try:
            ...
            self.assertEqual({'name': ['Поезд с таким номером уже существует.']}, e.message_dict)
        try:
            train = Train(name='t12', from_city=self.city_B, to_city=self.city_D, travel_time=8)
            train.full_clean()
        except ValidationError as e:
            self.assertEqual({'__all__': ['Исправте время в дороге']}, e.message_dict)

    def test_home_routes_view(self):
        response = self.client.get(reverse('home'))
        self.assertEqual(200, response.status_code)
        self.assertTemplateUsed(response, template_name='travels/home.html')
        self.assertEqual(views.home, response.resolver_match.func)

    def test_find_all_routes(self):
        graph = views.get_graph()
        all_ways = list(views.dfs_paths(graph, self.city_A.id, self.city_E.id))
    ...
    def test_messages_error_more_time(self):
        response = self.client.post('/route/find_route/', {'from_city': self.city_A.id,
                                                           'to_city': self.city_E.id,
                                                           'across_cities': [self.city_C.id],
                                                           'travelling_time': 10})
        self.assertContains(response, 'Время в дороге, больше заданого.', 1, 200)

    def test_messages_error_another_city(self):
        response = self.client.post('/route/find_route/', {'from_city': self.city_B.id,
                                                           'to_city': self.city_E.id,
                                                           'across_cities': [self.city_C.id],
                                                           'travelling_time': 20})
        self.assertContains(response, 'Маршрут, через ети города невозможен', 1, 200)
