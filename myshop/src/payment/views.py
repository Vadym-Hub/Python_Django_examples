...
def payment_process(request):
    ...
    if request.method == 'POST':
        ...
        result = braintree.Transaction.sale({'amount': f'{order.get_total_cost():.2f}',
...
def payment_done(request):
    """Обработчик перенаправления после успешной оплаты"""
    return render(request, 'payment/done.html')


def payment_canceled(request):
    """Обработчик перенаправления после неудачной оплаты"""
